...
type FirmwareConfig struct {
	...
	// CacheDir holds one directory per mirrored generation plus the
	// state file. Empty — the default — means "under StateDir", which
	// is where it belongs: unlike scan output (tmpfs, see
	// deploy/compose/scan-bridge.yml) this has to survive a restart, or
	// every reboot re-downloads ~1.7 MB and serves 503 until it
	// finishes.
	//
	// Deliberately NOT defaulted to a literal path. It was, and that
	// made the two settings silently independent: an operator who moved
	// paths.state_dir kept a firmware cache pointing at the old
	// location, which on a container that no longer has it fails at
	// startup with "create cache dir" — a message that names neither
	// the setting they changed nor the one they did not. Read it
	// through FirmwareCacheDir().
...
func Default() Config {
	return Config{
		...
		Firmware: FirmwareConfig{
			Enabled: true,
			// No CacheDir: it follows StateDir. See the field comment.
...
func (c *Config) Validate() error {
	...
	if c.Firmware.Enabled {
		// No emptiness check on its own: empty means "derive from
		// state_dir", and FirmwareCacheDir() does. Only having neither
		// is an error -- that would put the cache in a "firmware"
		// directory relative to the working directory.
		if c.Paths.StateDir == "" && c.Firmware.CacheDir == "" {
			return errors.New(
				"firmware.cache_dir must be set when paths.state_dir is empty")
		}
...
// FirmwareCacheDir is where the panel-firmware mirror keeps its
// generations: firmware.cache_dir when set, otherwise a "firmware"
// directory under paths.state_dir.
//
// The derivation is the point. These two settings describe one thing —
// where durable daemon state lives — and hard-coding the second one
// meant that moving the first silently left it behind.
func (c *Config) FirmwareCacheDir() string {
	if c.Firmware.CacheDir != "" {
		return c.Firmware.CacheDir
	}
	return filepath.Join(c.Paths.StateDir, "firmware")
}
