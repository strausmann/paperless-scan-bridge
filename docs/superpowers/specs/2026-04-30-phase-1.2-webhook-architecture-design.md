...
> ⚠️ **AMENDED 2026-08-06 — read the reconciliation first.** This spec predates
> ADRs 0005 (canonical `POST /scan` + bearer), 0009 (bridge ↔ sane-runtime
> over a Unix socket) and 0010 (profiles are declarative YAML). Where it
> describes a per-profile `POST /webhooks/<token>` trigger, a TCP/6566 SANE-net
> transport, or SQLite-backed profiles with a write CRUD API (DB deferred to
> Phase 1.4), it is **overridden** by
> [`../plans/2026-08-06-phase-1.2-adr-reconciliation.md`](../plans/2026-08-06-phase-1.2-adr-reconciliation.md)
...
**Image:** `gcr.io/distroless/static-debian12:nonroot` base (matches
the Phase-1.2 plan's Dockerfile and the `AGENTS.md` requirement
that production-track images run as non-root). Production-track
release builds **pin the base image by digest** in the Dockerfile
(rendered at release time by the CI pipeline that updates the
`FROM` line); developer / `:dev` images may use the
floating tag for convenience. Single Go binary
(`go build -tags netgo -ldflags '-s -w'`, `CGO_ENABLED=0`).
...
during smoke tests). Production-track release builds **pin the
base image by digest** (same convention as `scan-bridge`). Runs
the `saned` daemon as the unprivileged `saned` system user
(created by the `sane-utils` package; no root processes inside the
container).
...
#### Token-leak vectors and required mitigations

The token sits in the URL path, which means it travels through
several places that should NOT retain it:

- **Reverse-proxy access logs.** The deployment-side reverse
  proxy MUST suppress logging of the `/webhooks/...` path or
  redact the token segment. Recommended config snippets are
  shipped in the documentation pages for Traefik, Caddy, and
  nginx (Phase-1.2 docs include at least one example;
  contributors add more)
- **HTTP `Referer` header.** scan-bridge MUST NOT issue HTTP
  redirects from a `/webhooks/...` URL — a redirect to an
  external host would leak the URL via the `Referer` header.
  Phase-1.2 webhooks are pure POST endpoints with terminal
  status codes; no redirect path exists. This is enforced by
  the router's design, not by configuration
- **Metric labels.** No metric exposed at `/metrics` carries the
  token as a label value. Scan metrics use `profile` (the
  human-readable name) as the label, never the token. The
  metrics package is the single source-of-truth for label
  shapes; new metrics added in later phases inherit this rule
- **Structured logs.** The token is logged exactly once at
  profile creation / rotation (so the operator can copy it),
  prefixed with the profile name. After that, log lines for
  scans reference the **token prefix** (first 8 base64 chars)
  for correlation, never the full token
- **Browser history.** Webhook callers should POST with a JSON
  body, not paste the URL into a browser. The convenience
  `GET /webhooks/<token>?...` path documented in the Webhook
  API section is for `curl` and shell scripts, not interactive
  browser use; the documentation page calls this out
