...
**Branch from `main`, and prove it.** Run `git checkout main && git pull` immediately
before every `git checkout -b` — never branch from whatever happens to be checked out.
Before opening the PR, verify with `git log --oneline main..HEAD`: it must list only that
PR's own commits. A foreign commit there means the branch point is wrong; rebase onto
`main` before review. Two PRs sharing a commit merge without conflict and silently
duplicate the shared change — see `docs/learnings/lessons-learned.md`, 2026-08-27.
